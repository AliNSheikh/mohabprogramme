...
class sortemp extends StatelessWidget {
  String type;
  sortemp({required this.type});

  @override
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (context) => appcubit()..empbycom(type: type),
      child: BlocConsumer<appcubit, appstates>(
        listener: (context, state) {

          if(state is arcempsucc) {
            Navigator.of(context).push(AnimationsRoutefade(
                page: sortemp(type: type,)
            ),).then((value) { });
